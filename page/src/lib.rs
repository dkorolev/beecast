...
//!
//! Deliberately **nothing third-party**: the one dependency is the first-party, zero-dependency
//! `beecast-player`, so consumers with tiny dependency trees (`scsh` hand-rolls its JSON
//! on purpose) embed the pipeline without pulling serde or anyhow. The few JSON needs are met
